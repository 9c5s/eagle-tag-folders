...
async function fetchTree(): Promise<TreeNode[]> {
  const nodes = (await eagle.smartFolder.getAll()) as unknown as RawNode[];
...
<template>
  <ExcludedTreeSetting
    ...
    :excluded-ids="settings.excludedSmartFolderIds"
    @update:excluded-ids="(v) => (settings.excludedSmartFolderIds = v)"
  />
</template>
